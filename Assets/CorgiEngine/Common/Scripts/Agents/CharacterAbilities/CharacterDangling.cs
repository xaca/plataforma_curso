...
{	
	...
	public class CharacterDangling : CharacterAbility 
	{
		...
		protected virtual void Dangling()
		{
			...
			Vector3 raycastOrigin=transform.position + DanglingRaycastOrigin;
			if (!_character.IsFacingRight) { raycastOrigin=transform.position + Vector3.Scale(DanglingRaycastOrigin,_leftOne); }

			// we cast our ray downwards
			RaycastHit2D hit = MMDebug.RayCast (raycastOrigin,Vector3.down,DanglingRaycastLength,_controller.PlatformMask | _controller.OneWayPlatformMask | _controller.MovingOneWayPlatformMask,Color.black,_controller.Parameters.DrawRaycastsGizmos);			
