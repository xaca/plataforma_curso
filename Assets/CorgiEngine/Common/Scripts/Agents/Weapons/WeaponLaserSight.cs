...
{	
	...
	public class WeaponLaserSight : MonoBehaviour 
	{
		...
		protected virtual void ShootLaser()
		{
			...
			_hit = MMDebug.RayCast(_weapon.transform.position +  _weapon.transform.rotation * Vector3.Scale(RaycastOriginOffset,_direction), _weapon.transform.rotation * _direction, LaserMaxDistance, LaserCollisionMask, Color.red, true);

			// our laser will be shot from the weapon's laser origin
			_origin = _weapon.transform.position + _weapon.transform.rotation * Vector3.Scale (LaserOriginOffset, _direction);
			...
			{
				_destination = _weapon.transform.position;
				_destination.x = _destination.x + LaserMaxDistance * _direction.x;
				_destination = MMMaths.RotatePointAroundPivot (_destination, _origin, _weapon.transform.rotation);
			}
